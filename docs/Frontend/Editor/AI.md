...
在 **Plan 模式** 下，AI 助手会自动使用 `update_todos` 工具将制定的计划同步到侧边栏的 Todo 列表中。
- **实时同步**：AI 提出的每一步计划都会显示在列表里。
- **手动编辑**：你可以手动添加、删除或标记任务状态。
- **进度追踪**：支持 `pending` (待办)、`in_progress` (进行中) 和 `completed` (已完成) 三种状态。
