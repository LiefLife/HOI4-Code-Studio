/*
 * ErrorTip 入口：聚合规则，提供：
 *  - collectErrors(content): ErrorItem[]（用于右侧错误列表）
 *  - toDiagnostics(content): Diagnostic[]（用于 CodeMirror Lint）
 *  - createLinter(): Extension（直接接入编辑器）
 */
import type { ErrorItem, RangeItem, Rule, RuleResult } from './types'
...
/** 规则注册：后续可在此追加更多规则 */
const RULES: Rule[] = [alwaysRule, isRule, controlRule]

/**
 * 聚合执行所有规则
 */
function runAll(content: string): { errors: ErrorItem[]; ranges: RangeItem[] } {
  ...
  for (const rule of RULES) {
    const res: RuleResult = rule.apply(content, lines, lineStarts)
 ...
 * 导出：用于右侧错误列表的数据
 */
export function collectErrors(content: string): ErrorItem[] {
  return runAll(content).errors
}

/**
 * 将范围映射为 CodeMirror 诊断
 */
export function toDiagnostics(content: string): Diagnostic[] {
  const { ranges } = runAll(content)
  // 合并去重（按 from-to）
...
const errorLineField = StateField.define<DecorationSet>({
  create(state: EditorState) {
    ...
    const diags = toDiagnostics(text)
...
export function createLinter(): Extension[] {
  ...
  return [lintExt, gutter, errorLineField, errorLensField, theme]
}
