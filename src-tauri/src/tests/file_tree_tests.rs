...
mod tests {
    ...
    fn test_build_file_tree_with_valid_directory() {
        ...
        let file_name = test_path.file_name()
            .expect("路径应该包含文件名")
            .to_str()
            .expect("文件名应该是有效的UTF-8字符串");
        assert_eq!(root_node.name, file_name);
        assert!(root_node.is_directory, "根节点应该是目录");
        assert!(root_node.children.is_some(), "根节点应该有子节点");

        let children = root_node.children.as_ref()
            .expect("根节点应该有子节点");
    ...
    fn test_build_file_tree_with_file_instead_of_directory() {
        ...
        let test_file_str = test_file.to_str()
            .expect("测试文件路径应该是有效的UTF-8字符串");
    ...
    fn test_file_node_serialization() {
        ...
        let serialized = serde_json::to_string(&node)
            .expect("应该能够序列化FileNode");
        let deserialized: FileNode = serde_json::from_str(&serialized)
            .expect("应该能够反序列化FileNode");
    ...
    fn test_build_file_tree_fast_performance() {
        ...
        let test_path_str = test_path.to_str()
            .expect("测试路径应该是有效的UTF-8字符串");
        let result = build_file_tree_fast(test_path_str, 1);
